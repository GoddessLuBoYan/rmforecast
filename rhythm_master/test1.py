...
def get_md5_list():
    url = "http://game.ds.qq.com/" + ResPath + '/' + MD5ListPath
    res = request.urlopen(url)
    return res.read()
    if check_update():
        filename = os.path.join(data_path, 'MD5List.xml')
        with open(filename, 'wb') as f:
            f.write(res.read())
...
def check_update():
    flag = False
    newMD5 = get_md5_list()
    md5_filename = os.path.join(data_path, 'MD5List.xml')
    if not os.path.exists(md5_filename):
        ...
        flag = oldMD5 != newMD5
    if flag:
        print("There is an update in server!")
...
def check_update_async():
    _thread.start_new_thread(check_update_loop, ())
